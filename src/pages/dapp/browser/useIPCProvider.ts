  ...
  const handleTokenApproval = useRefCallback(
    async (
      ...
      try {
        ...
        sendError(event.id, { code: 4001, message: 'Transaction failed' });
      }
  ...
  const handleSendTransaction = useRefCallback(
    async (
      ...
      try {
        ...
        sendError(event.id, { code: 4001, message: (error as any).toString() });
  ...
  const handleSignMessage = useRefCallback(
    async (eventId: number, data: string, passphrase: string) => {
      try {
        ...
        sendError(eventId, { code: 4001, message: (error as any).toString() });
      }
    },
  );
      ...
      switch (event.name) {
        case 'requestAccounts':
          props.onRequestAddress(
            ...
            reason => {
              sendError(event.id, { code: 4001, message: reason });
            },
          );
          break;
        ...
        case 'signMessage':
          props.onRequestSignMessage(
            ...
            _ => {
              ...
              sendError(event.id, { code: 4001, message: 'Not implemented' });
            },
            reason => {
